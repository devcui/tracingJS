...
export abstract class BrowserAdapter
    ...
    constructor(strategy?: TracingStrategy) {
        this.clickEvent = fromEvent(win(), "click").pipe(
    ...
    start(): void {
        this.clickEvent.subscribe((event: Event) => {
            const data = this.collectingClicks(this.strategy.collectingClicksStrategy, event);
            if (data) {
                this.collect<Trace<TClick>>(this.strategy.collectorStrategy, Packet.create<TClick>(TraceType.Event, [TraceTag.Click, TraceTag.Event], data))
            }
        });
    }
