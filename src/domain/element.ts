import {TraceElementPosition} from "../tracker";
import {TracePoint} from "../types";
import {win} from "../utils";
import {HTMLProps, HTMLTag} from "./types";
...
export class TraceElement {
    ...
    get coordinate(): TraceElementPosition | undefined {
    ...
    get id(): string | null {
        const dataEventIdEl: HTMLElement | undefined = this.match([TracePoint.EVENT_ID]);
        if (dataEventIdEl) {
            return dataEventIdEl.getAttribute(TracePoint.EVENT_ID);
        }
        const dataEventTitleEl: HTMLElement | undefined = this.match([
            TracePoint.Title,
        ]);
        if (dataEventTitleEl) {
            return dataEventTitleEl.getAttribute("title");
        }
        const dataEventContainerEl: HTMLElement | undefined = this.match([
            TracePoint.Container,
        ]);
        if (dataEventContainerEl) {
            if (
                dataEventContainerEl.hasAttribute(TracePoint.EVENT_ID) ||
                dataEventContainerEl.hasAttribute(TracePoint.Title)
            ) {
                return (
                    dataEventContainerEl.getAttribute(TracePoint.EVENT_ID) ||
                    dataEventContainerEl.getAttribute(TracePoint.Title)
                );
            }
            const containerName: string | null = dataEventContainerEl.getAttribute(
                TracePoint.Container
            );
            if (containerName) {
    ...
    match(attributes: TracePoint[]): HTMLElement | undefined {
        ...
        const target: HTMLElement | undefined = elements.find(
            (element: HTMLElement) =>
    ...
    private extractEventTitle(): string | undefined {
        ...
        while (container && container != win().document.body) {
            if (container.hasAttribute(TracePoint.Container)) break;
        ...
        return !this.cursor.hasChildNodes() || tagName.toLowerCase() === HTMLTag.Svg
            ? this.getLeafElementTitle()
            : this.getNotLeafElementTitle();
    }

    private getLeafElementTitle(): string | undefined {
        if (!this.cursor) return undefined;
        const {tagName, textContent} = this.cursor;
        if (tagName.toLowerCase() === HTMLTag.Img)
            return this.cursor.getAttribute(HTMLProps.Alt) || undefined;
        if (tagName.toLowerCase() === HTMLTag.Svg) {
            const use: Element | undefined = Array.from(this.cursor.children).find(
                (childElement) => childElement.tagName.toLowerCase() === HTMLTag.Use
            );
            if (use) {
    ...
    private getNotLeafElementTitle(): string | undefined {
        ...
        const {length} = Array.from(this.cursor.children).filter((el: Element) =>
            el.hasChildNodes()
        );
    ...
    private isSimpleElement(children: HTMLCollection): boolean {
        ...
        if (elements.length > 0) {
            ...
            const els: Element[] = elements.filter((el) =>
                tags.includes(<HTMLTag>el.tagName.toLowerCase())
            );
